/*
    트리거(TRIGGER)
    1. DML(INSERT, UPDEAT, DELETE) 작업 후 자동으로 실행되는 데이터베이스 객체이다.
...
CREATE OR REPLACE TRIGGER TRIG2
    AFTER
    INSERT OR UPDATE OR DELETE      -- 순서 상관 없다.
    ON EMPLOYEES
    FOR EACH ROW
BEGIN
...
/*
    ...
    작업 순서
    1. 사원 테이블의 구조를 복사하여 퇴사자 테이블(RETIRES)을 생성한다.
    ...
    5. RETIRE_TRIG 트리거를 생성한다.
        1) AFTER DELETE 트리거
        2) OLD 테이블의 데이터를 모두 퇴사자 테이블 (RETIRES)로 삽입한다.
