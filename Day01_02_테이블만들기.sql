...
/*
    오라클의 데이터 타입
    1. CHAR(size)       : 고정 길이 문자 타입(size : 1 ~ 2000바이트) - 모두의 데이터 길이가 고정되어 있을 때 사용
    2. VARCHAR2(size)    : 가변 길이 문자 타입(size : 1 ~ 4000바이트)
    3. DATE             : 날짜/시간 타입
    4. TIMESTAMP        : 날짜/시간 타입(조금 더 정밀하다.)
    5. NUMBER(p,s)      : p(정밀도), s(스케일)로 표현하는 숫자 타입
        1) 정밀도 : 정수부와 소수부를 모두 포함하는 전체 유효 숫자가 몇 개인가?
        2) 스케일 : 소수부의 전체 유효 숫자가 몇 개인가?
        예시)
            (1) NUMBER      : 최대 38자리 숫자(22바이트) 표현 가능
            (2) NUMBER(3)   : 정수부가 최대 3자리인 숫자 (최대 999)
            (3) NUMBER(5,2) : 전체 5자리, 소수부 2자리 (예시 : 123.45)
            (4) NUMBER(2,2) : 1 미만의 소수부 2자리인 실수 (예시 : 0.15) - 정수부의 0은 유효 자리가 아니다.
*/
/*
    제약조건(Constraint)
    1. 널
