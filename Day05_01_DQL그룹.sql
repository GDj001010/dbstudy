/*
    ...
    3. GROUP BY절과 함께 집계함수를 사용할 수 있다.
*/

-- DISTINCT(중복제거)하듯 하나로 모아줌 GROUP BY는 행당 개수를 알고 있다.(함수 사용 가능)
...
SELECT 
  ...
  FROM 
       EMPLOYEES
 WHERE
       DEPARTMENT_ID IS NOT NULL        -- HAVING절도 가능하지만 성능상 WHERE절이 더 좋음, GROUP BY절을해야만 사용할 수 있는 조건은 HAVING절로 보낸다
 GROUP BY DEPARTMENT_ID                 -- GROUP BY절로 그룹화 할 대상(모수)이 적을 수록 성능에 유리하다.
HAVING COUNT(*) <= 5;   
...
/*
    조건 지정하기
    1. GROUP BY절로 그룹화 할 대상(모수)이 적을 수록 성능에 유리하다.
    2. GROUP BY 이전에 처리할 수 있는 조건은 WHERE절로 처리하는 것이 성능적으로 유리하다.
