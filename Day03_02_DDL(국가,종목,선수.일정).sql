...
-- 기본키 제거하기     -- DROP TABLE을 수행하면 제약조건도 모두 제거된다.
ALTER TABLE NATION_TBL
    DROP PRIMARY KEY;   -- 테이블의 기본키는 오직 1개이므로 제약조건의 이름을 몰라도 지울 수 있다.
--ALTER TABLE NATION_TBL
--    DROP CONSTRAINT PK_NATION;  -- 테이블의 기본키를 직접 명시하여 지움.
...
-- 외래키 제거하기      -- DROP TABLE을 수행하면 제약조건도 모두 제거된다.
ALTER TABLE PLAYER_TBL
...
-- 외래키 제약 조건 일시중지  (비활성화)
ALTER TABLE PLAYER_TBL
    DISABLE CONSTRAINT FK_PLAYER_EVENT;  -- 일시중지 
