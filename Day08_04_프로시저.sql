/*
    ...
    3. 형식
        CREATE [OR REPLACE] PROCEDURE 프로시저명[(매개변수)]  [] < 생략될 수 있음을 표시
        IS  -- AS도 가능
            변수선언
        BEGIN
            프로시저 본문
...
-- 프로시저4 호출(실행하기)
-- FNAME, LNAME 값을 저장할 변수를 선언한 뒤, 프로시저에 전달해야 한다.
DECLARE
    FNAME EMPLOYEES.FIRST_NAME%TYPE;
    LNAME EMPLOYEES.LAST_NAME%TYPE;
BEGIN
...
-- 5. 프로시저5 사원번호를 전달하면, 해당 사원의 FIRST_NAME이 출력 파라미터 FNAME에 저장되도록 작성.
...
BEGIN
    ...
    --  2) 제품 테이블의 재고 내역을 수정(UPDATE)한다.
    UPDATE PROD_TBL SET P_STOCK = P_STOCK - BUY_AMOUNT
    WHERE P_CODE = PCODE;
    --  3) 고객 테이블의 포인트를 수정(UPDATE)한다.
    --     총 구매액의 10%를 정수로 올림처리해서 포인트로 준다. CEIL(가격 * 구매수량 * 0.1)
...
EXCEPTION 
    WHEN OTHERS THEN            -- 모든 예외를 처리
        ...
        -- 실패한다면 롤백
        ROLLBACK;   -- 예외발생이 된다면 그 전 INSERT, UPDATE 작업들 취소.
END;
