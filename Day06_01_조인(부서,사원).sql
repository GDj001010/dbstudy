...
-- PK 값 먼저(드라이브테이블)  FK 값 나중(드리븐테이블)  FK값을 먼저 적어도 되지만 성능적으로 PK가 앞에 있어야 좋다.
--  2) 오라클 문법
SELECT E.EMP_NO, E.NAME, D.DEPT_NO, D.DEPT_NAME
...
-- 제약조건의 이름 : FK_EMP_DEPT
ALTER TABLE EMPLOYEE_TBL
      DISABLE CONSTRAINT FK_EMP_DEPT;

-- 외래키 제약조건이 없는 상태이므로, 제약조건(참조무결성)을 위배하는 데이터를 입력할 수 있다.
INSERT INTO EMPLOYEE_TBL(EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY)
VALUES(SEQ_EMPL.NEXTVAL, '김성실', 5, '대리', 'F', '98/12/01', 3500000);
COMMIT;
...
DELETE FROM EMPLOYEE_TBL
 WHERE EMP_NO = 1005; -- PK를 조건으로 사용하면 인덱스를 타기 때문에 빠르다, 인덱스를 타지 않는 일반 칼럼은 느리게 동작한다.
COMMIT;                    
