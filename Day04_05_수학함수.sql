...
-- 4. 부호 판별
--    SING(A) : A가 양수이면 1, A가 음수이면 -1, A가 0이면 0을 반환
SELECT
        SIGN(5)
      , SIGN(-5)
      , SIGN(0)
...
SELECT 
        ROUND(123.456)      -- 정수로 절사
      , ROUND(123.456, 1)   -- 소수 1자리 남기고 절사
      , ROUND(123.456, 2)   -- 소수 2자리 남기고 절사
      , ROUND(123.456, -1)  -- 일의 자리에서 절사
      , ROUND(123.456, -2)  -- 십의 자리에서 절사
  FROM DUAL;
