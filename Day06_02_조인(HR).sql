...
SELECT
  ...
  FROM DEPARTMENTS D INNER JOIN EMPLOYEES E
    ON D.DEPARTMENT_ID = E.DEPARTMENT_ID
 GROUP BY D.DEPARTMENT_NAME, D.DEPARTMENT_ID;  -- SELECT에서 보고싶은 칼럼은 무조건 GROUP BY절에 있어야 한다.
...
SELECT D.DEPARTMENT_NAME, COUNT(*)      -- 틀린 답안  모든 칼럼을 보는거기 때문에 사원정보가 NULL값이어도 1로 카운팅된다
  FROM DEPARTMENTS D LEFT OUTER JOIN EMPLOYEES E
    ON D.DEPARTMENT_ID = E.DEPARTMENT_ID
 GROUP BY D.DEPARTMENT_ID, D.DEPARTMENT_NAME;

SELECT D.DEPARTMENT_NAME, COUNT(E.EMPLOYEE_ID)  -- 사원정보만 가지고 카운팅 된다.
  FROM DEPARTMENTS D LEFT OUTER JOIN EMPLOYEES E
    ON D.DEPARTMENT_ID = E.DEPARTMENT_ID
 GROUP BY D.DEPARTMENT_ID, D.DEPARTMENT_NAME;
